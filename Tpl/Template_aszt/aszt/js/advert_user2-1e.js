...
﻿var User = {
    ...
    gameType:'',
	  gid:'10', //填写当前游戏ID
    gameUrl: {
      '1028' : indexUrl+'/game/login?game=10&server=10'
    },
    paramMap : {},
    type:["reg","login"],
   ...
   /*     var p = this.paramMap[key];
    ...
    quickYeyouReg: function () {
        ...
        if (User.isEmpty([cn, pwd,pwd2])) {
            ...
            //减少请求连接
        	alert(User.param("groupid"));
            if(cn_result[0] && pwd_result[0] && pwd2_result[0]){
            	pwd = hex_md5(pwd);
            	pwd2 = hex_md5(pwd2);
            	var paramObj = {
            			service : 'yeyouRegUser',
            			advert : 'stat',
            			enc : '0',
            			cn : cn,
            			pwd : pwd,
